...
import { createGuestServerClient } from "@/app/chat/lib/supabase/server-guest";
...
export async function GET(request: Request) {
  ...
  if (code) {
    ...
    if (!error) {
      ...
      return NextResponse.redirect(
        `${origin}/chat/auth/error?message=${encodeURIComponent(error.message)}`
      );
    }
  }
  ...
  return NextResponse.redirect(
    `${origin}/chat/auth/error?message=${encodeURIComponent(
