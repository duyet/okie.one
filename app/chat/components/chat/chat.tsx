...
import { Conversation } from "@/app/chat/components/chat/conversation"
import {
  checkRateLimits,
  createGuestUser,
  createNewChat,
  updateChatModel,
} from "@/app/chat/lib/api"
import {
  MESSAGE_MAX_LENGTH,
...
import {
  ...
  processFiles,
} from "@/app/chat/lib/file-handling"
import { API_ROUTE_CHAT } from "@/app/chat/lib/routes"
  ...
  const ensureChatExists = async () => {
    ...
    if (isFirstMessage) {
      try {
        ...
        setChatId(newChatId)
        if (propUserId) {
          window.history.pushState(null, "", `/chat/c/${newChatId}`)
        }
        return newChatId
      } catch (err: any) {
