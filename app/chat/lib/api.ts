import { APP_DOMAIN } from "@/app/chat/lib/config";
...
export async function checkUsage(supabase: SupabaseClient, userId: string) {
  ...
  console.log("userData", userData, userDataError);
...
export async function checkRateLimits(
  ...
  try {
    const res = await fetch(
      `/chat/api/rate-limits?userId=${userId}&isAuthenticated=${isAuthenticated}`,
      {
        method: "GET",
        headers: { "Content-Type": "application/json" },
      }
...
export async function signInWithGoogle(supabase: SupabaseClient) {
  try {
    ...
    const { data, error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${baseUrl}/chat/auth/callback`,
        queryParams: {
          access_type: "offline",
          prompt: "consent",
        },
