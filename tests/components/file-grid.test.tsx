...
describe("FileGrid", () => {
  ...
  it("should show actions on hover", async () => {
    ...
    const firstFileCard = screen
      .getByText("test-image.png")
    ...
    await waitFor(() => {
      ...
      const hasDownloadButton = downloadButtons.some(
        (button) => button.querySelector("svg.lucide-download") !== null
      )
  ...
  it("should render empty grid when no files", () => {
    ...
    const { container } = render(
      <FileGrid files={[]} onDelete={onDelete} onDownload={onDownload} />
    )
