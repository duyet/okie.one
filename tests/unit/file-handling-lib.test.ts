...
describe('File Handling Library', () => {
  ...
  describe('validateFile()', () => {
    ...
    it('should reject unsupported file types', async () => {
      ...
      expect(result.isValid).toBe(false)
      expect(result.error).toContain("File type not supported or doesn't match its extension")
    })
    ...
    it('should accept various supported file types', async () => {
      ...
      for (const type of supportedTypes) {
        mockFileTypeFromBuffer.mockResolvedValue({ mime: type.mime, ext: type.ext })
  ...
  describe('uploadFile()', () => {
    it('should upload file successfully', async () => {
        ...
        storage: {
          from: vi.fn().mockReturnValue({
            ...
            getPublicUrl: vi.fn().mockReturnValue({
              data: { publicUrl: 'https://storage.supabase.co/object/public/chat-attachments/uploads/abc123.txt' }
            })
          })
        }
      }
      ...
      expect(url).toBe('https://storage.supabase.co/object/public/chat-attachments/uploads/abc123.txt')
    ...
    it('should handle upload errors', async () => {
      ...
      await expect(uploadFile(mockSupabase as any, file)).rejects.toThrow('Error uploading file: Storage quota exceeded')
    })
  ...
  describe('createAttachment()', () => {
    ...
    it('should handle files with special characters', async () => {
      ...
      const file = new File(['content'], 'file name with spaces & symbols.txt', { 
        type: 'text/plain' 
      })
  ...
  describe('checkFileUploadLimit()', () => {
    ...
    it('should throw FileUploadLimitError when limit exceeded', async () => {
      ...
      vi.resetModules()
      const { checkFileUploadLimit, FileUploadLimitError } = await import('@/lib/file-handling')
      
      await expect(checkFileUploadLimit('user-123')).rejects.toThrow(FileUploadLimitError)
      await expect(checkFileUploadLimit('user-123')).rejects.toThrow('Daily file upload limit reached.')
    })
    ...
    it('should handle database errors', async () => {
      ...
      await expect(checkFileUploadLimit('user-123')).rejects.toThrow('Database connection failed')
    })
