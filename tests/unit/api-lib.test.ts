...
describe('API Library', () => {
  ...
  describe('OAuth Base URL Detection', () => {
    ...
    it('should handle GitHub OAuth sign-in successfully', async () => {
      ...
      const mockSupabase = {
        auth: {
          signInWithOAuth: vi.fn().mockResolvedValue({ data: { user: { id: '456' } }, error: null })
        }
      }
  ...
  describe('createGuestUser()', () => {
    ...
    it('should handle API errors gracefully', async () => {
      ...
      await expect(createGuestUser('invalid-id')).rejects.toThrow('Invalid user ID')
    })

    it('should handle network errors', async () => {
      const mockFetchClient = vi.fn().mockRejectedValue(new Error('Network error'))
      ...
      await expect(createGuestUser('guest-123')).rejects.toThrow('Network error')
    })
  })
  ...
  describe('checkRateLimits()', () => {
    ...
    it('should handle rate limit errors', async () => {
      ...
      await expect(checkRateLimits('user-123', true)).rejects.toThrow('Rate limit exceeded')
    })
  })
  ...
  describe('updateChatModel()', () => {
    it('should successfully update chat model', async () => {
      const mockResponse = {
        ok: true,
        json: vi.fn().mockResolvedValue({ chatId: 'chat-123', model: 'gpt-4', updated: true })
      }
      const mockFetchClient = vi.fn().mockResolvedValue(mockResponse)
      ...
      expect(result).toEqual({ chatId: 'chat-123', model: 'gpt-4', updated: true })
    })
    ...
    it('should handle update errors', async () => {
      ...
      await expect(updateChatModel('invalid-chat', 'gpt-4')).rejects.toThrow('Chat not found')
    })
  })
  ...
  describe('OAuth Sign-in Functions', () => {
    it('should handle OAuth errors gracefully', async () => {
      ...
      await expect(signInWithGoogle(mockSupabase as any)).rejects.toThrow('OAuth provider error')
    })

    it('should handle network errors during OAuth', async () => {
      const mockSupabase = {
        auth: {
          signInWithOAuth: vi.fn().mockRejectedValue(new Error('Network timeout'))
        }
      }

      const { signInWithGitHub } = await import('@/lib/api')

      await expect(signInWithGitHub(mockSupabase as any)).rejects.toThrow('Network timeout')
    })
  })
